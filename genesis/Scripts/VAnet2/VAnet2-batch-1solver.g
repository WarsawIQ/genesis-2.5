...
function setup_output
    ...
    str name, cell_name
    make_output "Vm_out"
    /* No per-cell Vm recording in this variant.
    **
    ** findsolvefield resolves a compartment's slot inside the solver that
    ** owns it, and with one solver per layer the per-cell addressing the
    ** stock script uses no longer applies. This variant exists to measure
    ** wall time and spike counts -- spikecount.g supplies the firing rate,
    ** and VAnet2-batch.g is where the recorded traces come from -- so the
    ** recording is simply omitted rather than reconstructed.
    */
    if(0)
    else
...
**
** The original idiom -- create a solver inside cell 0, then DUPLICATE it into
** every other cell -- exists because GENESIS refused a second spike generator
** in one solver. With that lifted, a layer is a single solver and the
...
** integration method the published run used, and dropping it would change the
** numbers for a reason that has nothing to do with the solver grouping.
**
** The per-solver x/y/z the original copied from each soma are dropped.
** connect_cells runs after this and addresses soma/spike and the synchans
** under the somas, not solvers, so nothing reads them.
**
** VAnet2-batch.g keeps the original idiom untouched: it produced the published
** timing and has to stay reproducible.
*/
if(hflag)
