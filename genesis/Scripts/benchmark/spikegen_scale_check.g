// genesis
// Does the accelerator still deliver synaptic input as the model grows?
//
// spikegen_syn_check.g with two cells passes on both arms. VAnet2 with 3200
// in one solver does not. This scales the same structure by NCELL so the
// failure can be bracketed: pairs of cells, the even one driven by injected
// current, the odd one driven only through a synapse from its partner.
//
...
int   NSTEP = 20000
int   NCELL = 2
int e_nc = {getenv GENESIS_BENCH_NCELL}
if ({e_nc} >= 2)
    NCELL = {e_nc}
end
...
for (i = 0; i < {NCELL}; i = i + 1)
    ...
    if ({i} % 2 == 1)
        ...
        addmsg /net/cell{i-1}/soma/spike {cell}/soma/syn SPIKE
        setfield {cell}/soma/syn synapse[0].weight 1.0 synapse[0].delay 0.0
    end
end

create spikehistory /spikeout
setfield /spikeout filename "spikegen_scale_check.txt" leave_open 1 ident_toggle 0
for (i = 0; i < {NCELL}; i = i + 1)
