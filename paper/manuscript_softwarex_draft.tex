...
\caption{Where GENESIS~2.5 sits among the other simulators on the Vogels--Abbott COBAHH network, one UMCS node. Every arm is single-threaded CPU except the two coloured apart, CoreNEURON and Arbor on an A100; the hatched bar is a single run, the rest means of 3 replicates. On the same core GENESIS is the fastest of the four --- two thirds of CoreNEURON's time as published, under half once built as one solver per layer --- but CoreNEURON on the GPU stays ahead of all of them.}
