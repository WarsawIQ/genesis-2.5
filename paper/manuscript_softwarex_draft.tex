...
\emph{Coverage.} The accelerator now runs a synaptically coupled spiking network, and whether it pays depends on size. GENESIS allowed a single spike generator per \texttt{hsolve}, so such a network was built as one solver per cell; lifting that lets the Vogels--Abbott model be built as one solver per layer, $1.56\times$ faster on CPU alone, with the device reproducing it to $0.35\%$ on spike count. A zero-delay network cannot batch, so every step pays a dispatch, and that cost is fixed while the CPU's grows with the population: the two arms cross near 5000 cells. At the benchmark's 4000 the GPU is $1.09\times$ slower; at 12\,500 it is $1.51\times$ faster. The published size sits just below the crossing.
