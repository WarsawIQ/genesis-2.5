...
\emph{Coverage.} Two things stand between the accelerator and a synaptically coupled network, and this release removes one. GENESIS allowed a single spike generator per \texttt{hsolve}, so such a network was built as one solver per cell; lifting that lets the Vogels--Abbott model be built as one solver per layer, which on its own runs $1.55\times$ faster on CPU ($31.8\pm0.4$\,s against $49.3\pm2.2$\,s). The second stands: the device's synaptic-channel opcode does not reproduce the CPU result, so a solver holding one is detected at \texttt{SETUP} and computed on the CPU. Profiling bounds what repairing it would be worth --- 59\% of that run is the interpreter and the Hines solve, a ceiling of $2.4\times$.
