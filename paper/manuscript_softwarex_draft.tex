...
\begin{abstract}
GENESIS is a long-established compartmental neural simulator. We release GENESIS~2.5, adding opt-in OpenCL and CUDA backends for the Hodgkin--Huxley channel-update path and \hte{}, a GPU Hines tree-elimination kernel for dendritic trees, leaving every existing model, script and MPI workflow unchanged. On NVIDIA A100 hardware it reaches $57\times$ end-to-end speedup on a 160\,000-compartment population, matching the fp64 CPU to $\sim$$10^{-7}$\,V. On CPU it runs the Vogels--Abbott COBAHH benchmark $1.63\pm0.07\times$ faster than CoreNEURON, and on GPU it overtakes the GPU-native Arbor beyond 76\,ms of simulated time, its cost per step being 32\% lower. The release also fixes seven latent defects and makes $O(n^2)$ model construction linear.
\end{abstract}
...
\textbf{CoreNEURON on the GPU.} CoreNEURON exists to run on accelerators, so a CPU-only comparison would flatter us. Building its OpenACC backend with NVHPC left NEURON itself unable to start, so we ran CoreNEURON on its own: NEURON writes the model out with \texttt{nrncore\_write}, and \texttt{special-core} reads those files and simulates without it, which keeps NEURON on its working GCC build. On one A100 CoreNEURON completes the same network in $27.0\pm0.1$\,s --- $2.83\times$ faster than its own CPU arm, and $1.74\times$ faster than GENESIS. Its spike count differs from the CPU arms by 6\% (592\,865 against 558\,824), the same activity regime, so the times remain comparable.
...
A broken $O(N)$ scheduler loop, re-scheduling neurons $2\ldots N$ every step, is fixed; that repair makes Section~3's throughput attainable.
