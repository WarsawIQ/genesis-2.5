...
\emph{Coverage.} The accelerator now runs a synaptically coupled spiking network. GENESIS allowed one spike generator per \texttt{hsolve}, so such a network was built as one solver per cell; lifting that lets the Vogels--Abbott model be built as one solver per layer, $1.56\times$ faster on CPU alone, with the device reproducing it to $0.35\%$ on spike count. A zero-delay network cannot batch, so every step pays a dispatch, which at this size outweighs what the kernel saves: the GPU arm is $1.09\times$ slower. Enlarging the network does not change that. Holding the in-degree at 80 per cell so the $27$\,Hz regime survives, 12\,500 cells give $127.1\pm1.7$\,s on the device against $130.7\pm2.5$ on the CPU, and the A100 matches within its scatter. The accelerable fraction stays near $59\%$ at any size, so the ceiling stays $2.4\times$ and the dispatch spends it. Reaching the profile's $2.4\times$ ceiling means cutting the dispatch, not growing the model.
