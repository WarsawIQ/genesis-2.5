...
\caption{Both simulators are linear in $K$, so they cross, and a comparison at a single run length reports whichever side of the crossing the author happened to choose. The crossing sits earlier on the A40 because our kernel is fp32 where Arbor computes in double, and that card's double-precision throughput is a fraction of the A100's. $N=10\,000$ neurons $\times$ 16 compartments, mean $\pm$ std over 3 replicates.}
\label{fig:crossover}
\end{figure}
...
\caption{Do the three simulators integrate the same model? One cell's first action potential, at the soma and at the far end of the dendrite, which moves only if axial coupling is integrated. Each simulator is drawn twice, one backend wide and pale with the other dashed on top, so a coinciding pair reads as agreement rather than a missing line. NEURON and Arbor agree to $0.02$\,mV at the peak and $0.01\%$ on rate; GENESIS peaks $4.6$\,mV lower, its Hodgkin--Huxley rates being written about a $-70$\,mV rest where NEURON's use $-65$. Data: \protect\path{cluster_bringup/logs/vmcross/}.}
\label{fig:vmequiv}
\end{figure}
