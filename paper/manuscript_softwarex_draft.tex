...
CoreNEURON exists to make NEURON models faster, discarding the interpreter and rebuilding the data layout for vectorisation, while the GENESIS arm uses no accelerator at all. CoreNEURON is in turn $1.25\times$ faster than NEURON~9.0.2, the expected ordering, which suggests that arm is configured correctly.
...
This is the boundary of what the present release offers. Built as one solver per layer the model runs on the accelerator, but only at parity with its own CPU arm (Section~5), so CoreNEURON on a GPU stays ahead --- by $1.23\times$ against our best CPU time, where it was $1.74\times$ against the published one. Where the accelerator pays --- multi-compartment populations --- the comparison below is GPU on both sides.
