...
\textbf{CUDA backend on NVIDIA hardware.} The CUDA backend builds from the same source (\texttt{make USE\_CUDA=1}) against the identical harness, and reproduces the fp64 CPU action potential to $7\times10^{-8}$\,V, confirming parity with both the CPU and OpenCL paths. Table~\ref{tab:cuda} reports a 10-replicate sweep on the two UMCS cluster GPUs. The two cards are close at every size, and the OpenCL backend trails CUDA by roughly $8\%$ --- about what a faithful port of the same kernels costs, and the price of running on hardware CUDA does not reach.
