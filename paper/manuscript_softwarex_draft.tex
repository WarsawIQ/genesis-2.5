...
\item Seven defect fixes. Three are in the Hines solver GENESIS~2.4 ships and reach every user of a multi-neuron \texttt{hsolve}, accelerator or not: \texttt{inject}-driven voltage transients were silently suppressed for all but the first neuron. The other four are in the accelerator, each returning wrong results rather than an error, and were exposed by validating against a published network model where synthetic benchmarks had shown nothing: compartments needing unimplemented opcodes reached the GPU because the check that detects them was never consulted; a degenerate branch topology was accelerated although documented as unsupported; accelerator state was held per device rather than per \texttt{hsolve}, corrupting any model with more than one solver; and the per-step channel update was dispatched once per solver instead of once per step.
