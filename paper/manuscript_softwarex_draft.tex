...
\emph{Coverage.} Both kernels implement the spike-generator and synaptic-channel opcodes, and per-step dispatch runs a spiking model on the device correctly. What keeps it from paying is a setup rule, not a kernel gap: GENESIS allows one spike generator per \texttt{hsolve}, so a spiking network becomes one solver per cell, paying a dispatch per cell per step. Allowing many generators in one solver leaves one dispatch per step, with no batching to interact with event ordering. Profiling bounds the prize: 59\% of that run is the interpreter and the Hines solve, so the ceiling is $2.4\times$; the synaptic element and event messaging are the rest.
