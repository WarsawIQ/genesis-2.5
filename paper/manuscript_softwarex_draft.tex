...
\begin{abstract}
GENESIS is a long-established compartmental neural simulator. We release GENESIS~2.5, adding opt-in OpenCL and CUDA backends for the Hodgkin--Huxley channel-update path and \hte{}, a GPU Hines tree-elimination kernel for dendritic trees, leaving every existing model, script and MPI workflow unchanged. On cluster hardware it reaches $57\times$ end-to-end speedup on a 160\,000-compartment population, matching the fp64 CPU to $\sim$$10^{-7}$\,V. On CPU it runs the Vogels--Abbott COBAHH benchmark $1.63\pm0.07\times$ faster than CoreNEURON, and on GPU it overtakes the GPU-native Arbor beyond 64\,ms of simulated time; on spiking networks, which it cannot yet accelerate, CoreNEURON's GPU stays $1.74\times$ ahead. It also fixes seven latent defects and makes $O(n^2)$ model construction linear.
\end{abstract}
...
\item Seven defect fixes. Four are in the accelerator, each returning silently wrong results rather than an error: compartments needing unimplemented opcodes were dispatched to the GPU because the check that detects them was never consulted; a degenerate branch topology was accelerated although documented as unsupported; accelerator state was held per device rather than per \texttt{hsolve}, corrupting any model with more than one solver; and the per-step channel update was dispatched once per solver instead of once per step. Three are in the Hines solver, where they suppressed \texttt{inject}-driven transients in any multi-neuron \texttt{hsolve}, and affect existing users regardless of GPU use.
...
GENESIS 2.5 closes this gap: GENESIS 2.4/PGENESIS 2.4 plus an OpenCL backend for the \texttt{hsolve} channel-update path and a CUDA backend on the same interface, validated on UMCS cluster A100/A40 nodes (Section~3).
...
\begin{enumerate}
    ...
    One boundary follows from the architecture rather than from the kernels: GENESIS gives each \texttt{hsolve} a single spike generator, so a synaptically coupled spiking network is built as one solver per cell, and per-dispatch cost then dominates. Such models run correctly, on the CPU, and Section~3 measures what that costs against a simulator that does accelerate them.
...
\textbf{Reproducing the CPU baseline.} \texttt{nxgenesis} on the bundled \texttt{Cal8.g} and \texttt{Cal7difshell.g} models gives stable, low-variance timings (CV $\approx 1\%$, 30 replicates), confirming ordinary workflows are unaffected.
...
\hline
GENESIS 2.5 (VAnet2) & tabulated & 46.9 $\pm$ 2.1 & 536\,600 \\
\hline
...
\caption{GENESIS~2.5 against NEURON and CoreNEURON on the Vogels--Abbott COBAHH network, one UMCS node. All arms are single-threaded CPU except the top one, CoreNEURON on an A100, coloured apart. Hatched bars are single runs; the others are means of 3 replicates with standard deviations. Throughput is quoted over the $4\times10^{8}$ neuron-steps the 5\,s simulation advances. Against CoreNEURON on CPU, the GENESIS arm completes the network in two thirds of the time using no accelerator at all; against CoreNEURON on the GPU it is $1.74\times$ slower, because a spiking network of single-compartment cells is the one case this release cannot accelerate.}
...
\caption{The same model in three simulators: membrane potential of one cell over its first action potential, at the soma where the current is injected and at the centre of the last dendrite, which moves only if axial coupling is integrated. Each simulator is drawn twice, wide and pale for one backend with the other dashed on top, so a pair that coincides reads as agreement rather than as a missing line. NEURON and Arbor lie on each other to $0.02$\,mV at the peak and give the same firing rate to $0.01\%$. GENESIS peaks $4.6$\,mV lower and $0.24$\,ms earlier: its Hodgkin--Huxley rates are written about a $-70$\,mV resting potential and NEURON's \texttt{hh} about $-65$\,mV, so GENESIS's $\alpha_m(v)$ equals NEURON's $\alpha_m(v{+}5)$. Its own two backends agree to $2.6\times10^{-5}$\,V over 200\,ms and 14 spikes, fp32 against fp64. Measuring this exposed three defects in the harness --- Arbor injecting into the middle of the cable rather than the soma, Arbor using NEURON's default reversal potentials instead of the model's, and GENESIS scaling every compartment's channel conductance by the soma's area --- all fixed here; none of them changed any wall time by more than $1.3\%$. Data: \protect\path{cluster_bringup/logs/vmcross/}.}
\label{fig:vmequiv}
\end{figure}
...
The release also carries seven correctness fixes. Three are latent Hines-solver defects that silently suppressed \texttt{inject}-driven voltage transients in any multi-neuron \texttt{hsolve}; these benefit any GENESIS/PGENESIS user, accelerator or not. The other four are in the accelerator and are listed in Section~1. Separately, five pre-existing linear-scan patterns compounding into $O(n^2)$ model-construction time are fixed, restoring linear scaling and making population-scale models buildable at all.
...
\emph{Coverage.} The CUDA kernel implements the spike-generator and synaptic-channel opcodes, and per-step dispatch runs a spiking model on the device correctly; the OpenCL kernel does not yet. What keeps that path from paying is a setup rule: GENESIS allows one spike generator per \texttt{hsolve}, so a spiking network becomes one solver per cell, paying a dispatch per cell per step. Allowing many in one solver leaves one dispatch per step, with no batching to interact with event ordering. Profiling bounds the prize: 59\% of that run is the interpreter and the Hines solve, so the ceiling is $2.4\times$; the synaptic element and event messaging are the rest.

\emph{Comparison.} CoreNEURON's multi-rank MPI configuration, and Arbor on a spiking network, are still unmeasured and would place GENESIS~2.5 more completely.
...
During the preparation of this work the authors used Anthropic's Claude, via the Claude Code interface, to draft and edit manuscript text, to write the benchmarking harnesses and analysis scripts under \path{cluster_bringup/} and \path{paper/scripts/}, and to assist with implementing and debugging the accelerator backends. Every result reported here was produced by executing the software on the hardware named in the text, and the measurements, the defects reported, and the conclusions drawn from them were checked by the authors against the raw data included in the repository. The authors reviewed and edited all output, and take full responsibility for the content of the published article.
