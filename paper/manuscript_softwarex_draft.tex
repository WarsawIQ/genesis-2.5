...
\emph{Coverage.} The accelerator now runs a synaptically coupled spiking network, and whether it pays depends on size. GENESIS allowed a single spike generator per \texttt{hsolve}, so such a network was built as one solver per cell; lifting that lets the Vogels--Abbott model be built as one solver per layer, $1.56\times$ faster on CPU alone, with the device reproducing it to $0.35\%$ on spike count. A zero-delay network cannot batch, so every step pays a dispatch, which at this size outweighs what the kernel saves: the GPU arm is $1.09\times$ slower. That cost is fixed while the CPU's grows with the population, but this benchmark cannot be enlarged to show it: connectivity is a fixed $2\%$ probability, so more cells means a denser network, and firing collapses from $27$\,Hz to under $1$. Reaching the profile's $2.4\times$ ceiling means cutting the dispatch, not growing the model.
