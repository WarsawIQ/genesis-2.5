...
\emph{Coverage.} The accelerator now runs a synaptically coupled spiking network. GENESIS allowed a single spike generator per \texttt{hsolve}, so such a network was built as one solver per cell; lifting that lets the Vogels--Abbott model be built as one solver per layer, which on CPU alone is $1.56\times$ faster ($31.5\pm0.4$\,s against $49.3\pm2.2$\,s), and the device then reproduces it to $0.35\%$ on spike count. It is not yet worth using: at $41.1\pm1.6$\,s the GPU arm is $1.30\times$ slower than that CPU arm, because a zero-delay network cannot batch and each of the $10^5$ steps pays a dispatch. Reducing that, not correctness, is what stands between here and the $2.4\times$ ceiling the profile allows.
